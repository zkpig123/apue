//test: whether flock could mandatorily lock a file, need
...
//conclude: flock supply advisory lock! as man page said

#include "assist.h"

#define FILE_PATH_TO_LOCK "mandatory_flock.test"
...
int main (void)
{
	...
	while (flock(fd, LOCK_EX) == -1){
		if (errno != EINTR) p_err("flock failed.");
	}
	...
	getchar();
	if (flock(fd, LOCK_UN) == -1) p_err("flock release failed.");
